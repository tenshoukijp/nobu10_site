...
if ($_SERVER['HTTP_HOST']=="usr.s602.xrea.com") {
    if (str_contains($_SERVER["REQUEST_URI"], "/xn--rssx72hmcj.jp-mod.net")) {
        $OldRequest = $_SERVER["REQUEST_URI"];
        $NewRequest = str_replace("/xn--rssx72hmcj.jp-mod.net", "", $OldRequest);
        $NewUrl = "https://xn--rssx72hmcj.jp-mod.net".$NewRequest;
...
$strPageTemplate = preg_replace_callback("/(<img src=[\"'])([^\"']+?)([\"'])((\s+class=[\"'][^\"']+?[\"'])?(\s+attr=[\"']noedge[\"'])?(\s+align=[\"'][a-z]+[\"'])?(\s+attr=[\"']noedge[\"'])?>)/",
   function ($matches) {
       ...
       // サイト内の画像
           ...
           $strTargetImageFileName = "/virtual/usr/public_html/xn--rssx72hmcj.jp-mod.net/" . $matches[2];
...
// トップページのテンプレートの読み込み
$strIndexTemplate = file_get_contents("index_STR.html");
...
if (isset($matchesTitle[1])) {
    ...
    if ($lvsSimilarity > 0.7) {
        $strTitle = "蒼天録.jp | " . $strH2ContentCleaned;
    } else if (strlen($strParentDir)>1) {
        $strTitle = "蒼天録.jp | " . $strParentDir . " | " . $strH2ContentCleaned;
    } else {
        $strTitle = "蒼天録.jp | " . $strH2ContentCleaned;
    }
}
